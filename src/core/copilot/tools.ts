/**
 * Tool definitions for GitHub Copilot (OpenAI function calling format).
 *
 * These mirror Claudian's core tools but in the format Copilot expects.
 */

import type { CopilotToolDefinition } from './client';
...
export const COPILOT_TOOL_DEFINITIONS: CopilotToolDefinition[] = [
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          path: { type: 'string', description: 'Directory or file to search in, relative to vault root. Defaults to vault root.' },
          include: { type: 'string', description: 'Glob pattern to filter files, e.g. "*.ts" or "*.md"' },
          context_lines: { type: 'number', description: 'Number of context lines before and after each match. Default 0.' },
        },
        required: ['pattern'],
      },
    },
  },
  {
    type: 'function',
    function: {
      name: COPILOT_TOOL_NAMES.GLOB,
      description:
        'Find files matching a glob pattern. Returns a list of matching file paths. Use for discovering project structure.',
      parameters: {
        type: 'object',
        properties: {
          pattern: { type: 'string', description: 'Glob pattern, e.g. "**/*.ts", "src/**/*.md", "*.json"' },
          path: { type: 'string', description: 'Directory to search in, relative to vault root. Defaults to vault root.' },
        },
        required: ['pattern'],
      },
    },
  },
  {
    type: 'function',
    function: {
      name: COPILOT_TOOL_NAMES.LS,
      description: 'List the contents of a directory. Shows files and subdirectories with their types.',
      parameters: {
        type: 'object',
        properties: {
          path: { type: 'string', description: 'Directory path relative to vault root. Defaults to vault root.' },
        },
        required: [],
      },
    },
