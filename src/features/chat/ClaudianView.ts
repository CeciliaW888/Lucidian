...
export class ClaudianView extends ItemView {
  ...
  private buildHeader(header: HTMLElement) {
    ...
    // Logo (hidden when 2+ tabs) - Lucidian crystal mascot
    ...
    // Render all paths for the crystal mascot
    for (const pathData of LOGO_SVG.paths) {
      ...
      if (pathData.opacity) path.setAttribute('opacity', pathData.opacity);
      svg.appendChild(path);
    }
    this.logoEl.appendChild(svg);

    // Title text (hidden in header mode when 2+ tabs)
    this.titleTextEl = this.titleSlotEl.createEl('h4', { text: 'Lucidian', cls: 'claudian-title-text' });
