...
export class ClaudianSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl)
      .setName('Backend provider')
      .setDesc('Choose between Claude Code (native SDK) and GitHub Copilot')
      .addDropdown((dropdown) => {
        dropdown
    ...
    if (this.plugin.settings.provider === 'copilot') {
      this.displayCopilotSettings(containerEl);
    }
  ...
  private displayCopilotSettings(containerEl: HTMLElement): void {
    // Authentication status
    if (this.plugin.settings.copilotPat) {
      new Setting(containerEl)
        .setName('Copilot status')
        .setDesc('Authenticated with GitHub Copilot')
        .addButton((btn) =>
          btn.setButtonText('Sign out').onClick(async () => {
            this.plugin.settings.copilotPat = '';
            await this.plugin.saveSettings();
            this.display();
          }),
        );
    } else {
      const authStatusEl = containerEl.createDiv({ cls: 'claudian-copilot-auth-status' });
      ...
      new Setting(containerEl)
        ...
        .addButton((btn) =>
          btn
            ...
            .setCta()
            .onClick(async () => {
              await this.startCopilotAuth(authStatusEl);
            }),
        );
    }
    ...
    new Setting(containerEl)
      ...
      .addDropdown((dropdown) => {
        ...
        dropdown
          .setValue(this.plugin.settings.copilotModel)
          .onChange(async (value) => {
  ...
  private async startCopilotAuth(statusEl: HTMLElement): Promise<void> {
    try {
      const deviceCode = await fetchDeviceCode();

      statusEl.empty();
      const instructionsEl = statusEl.createDiv({ cls: 'claudian-copilot-auth-instructions' });
      instructionsEl.createEl('p', {
        text: `1. Copy this code: `,
      }).createEl('strong', { text: deviceCode.user_code });
      const linkP = instructionsEl.createEl('p');
      linkP.appendText('2. Open ');
      linkP.createEl('a', {
        href: deviceCode.verification_uri,
        text: deviceCode.verification_uri,
      });
      linkP.appendText(' and paste it');
      instructionsEl.createEl('p', { text: '3. Waiting for authorization...' });
      ...
      this.plugin.settings.copilotPat = pat;
      await this.plugin.saveSettings();
